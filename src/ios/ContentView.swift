...
func check() {
    ...
    if notfirstuse {
        ...
        //print("directory :\(directory.path())")
        let fileURL=directory.appending(path : "rand.txt")
        let randcontents=try! String(contentsOf: fileURL,encoding: .utf8)
        let randlines=randcontents.split(separator: "\n")
        randomSecret=String(randlines[0])
        user=String(randlines[1])
        
        let siteURL=directory.appending(path: "sites.txt")
        let siteContents=try! String(contentsOf: siteURL,encoding: .utf8)
        ...
        userprompt="Enter default username"
        print("random field")
        //userEnabled=true
    }
}
                ...
                VStack(alignment : .leading,spacing: 8) {
                        ...
                        HStack {
                            ...
                            TextField("Username",text:$username)
                                ...
                                .background((userEnabled || !notfirstuse) ? Color.yellow : Color.gray.opacity(0.2))
                            //.border(.secondary)
                        ...
                        HStack{
                            Text("Service")
                            TextField("to be added or deleted",text:$service)
